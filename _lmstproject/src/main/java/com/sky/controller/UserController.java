...
public class UserController {
	...
	public String listMemberuser(HttpServletRequest request, Model model,
		...
		List<UserDto> userList1 = userService.getUserListWithPaging(page, size);
	...
	public String processUpdateForm(@PathVariable("loginId") String loginId, @ModelAttribute("updatedUser") UserDto updatedUser, HttpSession session) {
	...
	public String showUpdateadmin(@PathVariable("loginId") String loginId, Model model) {
		UserDto user2 = userService.getUserById(loginId);
		model.addAttribute("user2", user2);
		return "updateadmin";
	}

	@PostMapping("/memberList/{loginId}/updateadmin")
	public String processUpdateadmin(@PathVariable("loginId") String loginId, @ModelAttribute("updatedUser") UserDto updatedUser, HttpSession session) {
		// 받아온 수정된 회원 정보에 번호를 설정
		updatedUser.setLoginId(loginId);
