...
<table id="videoList" class="table">
    ...
    <c:forEach items="${videos}" var="video" varStatus="loop">
        <tr class="videoItem" data-videoId="${video.videoId}">
            <td>${video.conNum}</td>
            <td><img src="${video.thumbUrl}" alt="thumbnail" class="thumbnail"></td>
            <td>${video.conName}</td>
            <td>${video.conDesc}</td>
            <td>
                <ul class="chapter-list">
                    ...
                    <c:forEach items="${chapters}" var="chapter">
                        <!-- chapter테이블의 conNum과  -->
                        <c:if test="${chapter.conNum eq video.conNum}">
...
<script>
    ...
    function openPopup(index) {
        //팝업창 안에서 사용할 객체들 선언.
        currentVideoId = window.videos[index].videoId
        var conName = window.videos[index].conName;
        var chapters = window.videos[index].chapters;
